...
export default async function SupervisorOperationsPage({ params }: { params: Promise<{ locale: string; id: string }> }) {
  ...
  return <AdminPageLayout locale={locale} title={`ملف المشرف: ${String(supervisorData.name||"")}`}><SupervisorMonthlyWork supervisor={serial(supervisorData)} trainees={clean(traineesSnap.docs)} activities={clean(activitiesSnap.docs)} sessions={clean(sessionsSnap.docs)} minutes={clean(minutesSnap.docs)} documents={clean(documentsSnap.docs)}/><div id="schedule" style={{scrollMarginTop:20}}><AdminSupervisorOperations locale={locale} supervisor={supervisorData} initialSlots={slots.docs.map((doc)=>({id:doc.id,...doc.data()}))}/></div></AdminPageLayout>;
}
