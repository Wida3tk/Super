...
export default async function SupervisorOperationsPage({ params }: { params: Promise<{ locale: string; id: string }> }) {
  ...
  return <AdminPageLayout locale={locale} title={`ملف المشرف: ${String(supervisorData.name||"")}`}><SupervisorWorkHistory supervisor={supervisorData} trainees={traineesSnap.docs.map(d=>({id:d.id,...d.data()}))} activities={activitiesSnap.docs.map(d=>({id:d.id,...d.data()}))} sessions={sessionsSnap.docs.map(d=>({id:d.id,...d.data()}))} minutes={minutesSnap.docs.map(d=>({id:d.id,...d.data()}))} documents={documentsSnap.docs.map(d=>({id:d.id,...d.data()}))}/><div id="schedule" style={{scrollMarginTop:20}}><AdminSupervisorOperations locale={locale} supervisor={supervisorData} initialSlots={slots.docs.map((doc)=>({id:doc.id,...doc.data()}))}/></div></AdminPageLayout>;
}
