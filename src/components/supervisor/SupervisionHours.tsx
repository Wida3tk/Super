...
const COLORS = {
  ...
  success: "#10B981", warning: "#EF9F27", danger: "#EF4444",
...
// Modal تسجيل جلسة
// ===========================
function SessionModal({ trainees, onClose, onSubmit }: {
  trainees: Trainee[];
  onClose: () => void;
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "1rem" }}
      ...
      <div style={{ background: "#fff", borderRadius: 16, padding: "1.5rem", width: "100%", maxWidth: 480, maxHeight: "90vh", overflowY: "auto", direction: "rtl" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 6 }}>
            {(["individual", "group", "absence", "warning"] as SessionType[]).map(t => (
              <button key={t} onClick={() => setType(t)}
                style={{ padding: "8px 4px", border: `1px solid ${type === t ? typeConfig[t].color : COLORS.gray300}`, borderRadius: 8, cursor: "pointer", fontSize: 12, fontWeight: type === t ? 600 : 400, background: type === t ? typeConfig[t].bg : "#fff", color: type === t ? typeConfig[t].color : COLORS.gray500 }}>
                {typeConfig[t].label}
              </button>
            ))}
          </div>
        ...
        {(type === "individual" || type === "absence" || type === "warning") && (
          <div style={{ marginBottom: "1rem" }}>
            ...
            <select value={selectedTrainee} onChange={e => setSelectedTrainee(e.target.value)}
              ...
              {trainees.map(t => <option key={t.id} value={t.id}>{t.name} — {t.license}</option>)}
            </select>
          </div>
        )}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {trainees.map(t => (
                <div key={t.id} onClick={() => toggleTrainee(t.id)}
                  style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", border: `1px solid ${selectedTrainees.includes(t.id) ? COLORS.primary : COLORS.gray300}`, borderRadius: 8, cursor: "pointer", background: selectedTrainees.includes(t.id) ? "#E6F1FB" : "#fff" }}>
        ...
        {type === "absence" && (
          <div style={{ marginBottom: "1rem" }}>
            ...
            <select value={absenceReason} onChange={e => setAbsenceReason(e.target.value as AbsenceReason)}
        ...
        {type === "warning" && (
          <div style={{ marginBottom: "1rem" }}>
            ...
            <select value={warningReason} onChange={e => setWarningReason(e.target.value as WarningReason)}
              style={{ width: "100%", padding: "8px 10px", fontSize: 13, border: `1px solid ${COLORS.gray300}`, borderRadius: 8 }}>
        ...
        <div style={{ marginBottom: "1rem" }}>
          ...
          <input type="text" value={notes} onChange={e => setNotes(e.target.value)} placeholder="أضف ملاحظة..."
            style={{ width: "100%", padding: "8px 10px", fontSize: 13, border: `1px solid ${COLORS.gray300}`, borderRadius: 8 }} />
        </div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={handleSubmit} disabled={!canSubmit || loading}
            style={{ flex: 1, padding: 10, border: "none", borderRadius: 8, cursor: canSubmit ? "pointer" : "not-allowed", fontSize: 13, fontWeight: 600, background: !canSubmit ? COLORS.gray200 : type === "absence" ? "#BA7517" : type === "warning" ? "#A32D2D" : COLORS.primary, color: !canSubmit ? COLORS.gray500 : "#fff" }}>
...
function TraineeCard({ trainee, snapshot, onSelect }: { trainee: Trainee; snapshot: any; onSelect: () => void }) {
  ...
  return (
    <div onClick={onSelect}
      style={{ background: "#fff", borderRadius: 12, padding: "14px 16px", border: `1px solid ${COLORS.gray200}`, cursor: "pointer", marginBottom: 10, display: "flex", alignItems: "center", gap: 12 }}
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
          ...
          <span style={{ fontSize: 11, color: COLORS.gray500, background: COLORS.gray100, padding: "2px 8px", borderRadius: 99, border: `1px solid ${COLORS.gray200}` }}>{trainee.license}</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ flex: 1, height: 6, background: COLORS.gray200, borderRadius: 99, overflow: "hidden" }}>
            <div style={{ height: "100%", borderRadius: 99, width: `${pct}%`, background: pct >= 100 ? COLORS.success : COLORS.primary }} />
          </div>
          <span style={{ fontSize: 11, color: COLORS.gray500, minWidth: 60 }}>{total}/{trainee.requiredHours} ساعة</span>
        </div>
        <div style={{ display: "flex", gap: 12, marginTop: 4 }}>
          <span style={{ fontSize: 11, color: COLORS.gray500 }}>🎯 فردية: <strong>{snapshot?.individualHours || 0}</strong></span>
          ...
          {(snapshot?.absenceCount || 0) > 0 && <span style={{ fontSize: 11, color: COLORS.warning }}>🚫 غياب: <strong>{snapshot.absenceCount}</strong></span>}
        </div>
      </div>
...
// المكوّن الرئيسي
// ===========================
export default function SupervisionHours({ supervisorId, initialTrainees = [], initialSessions = [], initialSnapshots = [] }: {
  supervisorId: string;
  initialTrainees?: any[];
  initialSessions?: any[];
  ...
  const getTraineeSnapshot = useCallback((traineeId: string) => {
    return snapshots.find(s => s.traineeId === traineeId) || null;
  }, [snapshots]);

  const monthTotals = useCallback(() => {
    const sessionSet = new Set<string>();
    let individual = 0, group = 0;
    sessions.forEach(s => {
      if (s.type === "individual") individual += s.duration || 0;
      if (s.type === "group" && !sessionSet.has(s.id)) { group += s.duration || 0; sessionSet.add(s.id); }
    });
    ...
    return (
      <div style={{ direction: "rtl" }}>
        ...
        <div style={{ background: "#fff", borderRadius: 14, border: `1px solid ${COLORS.gray200}`, overflow: "hidden" }}>
          <div style={{ padding: "12px 16px", borderBottom: `1px solid ${COLORS.gray200}`, display: "flex", justifyContent: "space-between", background: COLORS.gray100 }}>
          ...
          {traineeSessions.length === 0 ? (
            <div style={{ padding: "2rem", textAlign: "center", color: COLORS.gray500, fontSize: 13 }}>لا توجد جلسات مسجّلة هذا الشهر</div>
          ) : (
                ...
                {traineeSessions.map(s => (
                  <tr key={s.id} style={{ borderBottom: `1px solid ${COLORS.gray200}` }}>
                    ...
                    <td style={{ padding: "11px 14px" }}>
                      <span style={{ fontSize: 11, padding: "3px 9px", borderRadius: 99, background: s.type === "individual" ? "#E6F1FB" : s.type === "group" ? "#E0F7FC" : s.type === "absence" ? "#FAEEDA" : "#FCEBEB", color: s.type === "individual" ? "#185FA5" : s.type === "group" ? "#0E6E7A" : s.type === "absence" ? "#854F0B" : "#791F1F" }}>
                        {s.type === "individual" ? "فردية" : s.type === "group" ? "جماعية" : s.type === "absence" ? "غياب" : "إنذار"}
                      </span>
                    </td>
                    <td style={{ padding: "11px 14px", fontSize: 13, fontWeight: 500 }}>{s.duration ? `${s.duration} ساعة` : "—"}</td>
  ...
  return (
    <div style={{ direction: "rtl" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.25rem" }}>
        ...
        <button onClick={() => setShowModal(true)}
          style={{ background: COLORS.deep, color: "#fff", border: "none", borderRadius: 10, padding: "9px 18px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
          + تسجيل جلسة
        </button>
      </div>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: "1.25rem", overflowX: "auto", paddingBottom: 4 }}>
        {months.map(m => (
          <button key={m} onClick={() => setSelectedMonth(m)}
            style={{ padding: "6px 14px", borderRadius: 99, fontSize: 12, cursor: "pointer", whiteSpace: "nowrap", border: `1px solid ${selectedMonth === m ? COLORS.primary : COLORS.gray300}`, background: selectedMonth === m ? COLORS.deep : "#fff", color: selectedMonth === m ? "#fff" : COLORS.gray500, fontWeight: selectedMonth === m ? 600 : 400 }}>
            {formatMonth(m)}
          </button>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 10, marginBottom: "1.25rem" }}>
        {[{ label: "إجمالي الساعات", value: totals.total, color: COLORS.primary }, { label: "فردية", value: totals.individual, color: COLORS.primary }, { label: "جماعية", value: totals.group, color: "#0891b2" }, { label: "المتدربون النشطون", value: trainees.length, color: COLORS.success }].map(s => (
      ...
      <div style={{ background: "#fff", borderRadius: 14, border: `1px solid ${COLORS.gray200}`, overflow: "hidden" }}>
        ...
        <div style={{ padding: "12px 16px" }}>
          {trainees.length === 0 ? (
            <div style={{ padding: "2rem", textAlign: "center", color: COLORS.gray500, fontSize: 13 }}>لا يوجد متدربون مسندون إليك حالياً</div>
          ) : (
            trainees.map(t => <TraineeCard key={t.id} trainee={t} snapshot={getTraineeSnapshot(t.id)} onSelect={() => setSelectedTrainee(t)} />)
          )}
        </div>
      </div>
