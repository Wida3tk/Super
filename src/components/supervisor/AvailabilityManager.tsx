...
export default function AvailabilityManager({ supervisorId, locale }: Props) {
  ...
  const fetchSlots = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/availability/list?supervisorId=${supervisorId}`,
      );
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      const errMap: Record<string, string> = {
        DATE_IN_PAST: "التاريخ في الماضي",
  ...
  return (
    <>
      <style>{`
        .am-wrap { display: grid; grid-template-columns: 255px minmax(0,1fr); gap: 12px; height:100%; }
        @media(max-width: 900px) { .am-wrap { grid-template-columns: 1fr; } }

        .am-card { background: #fff; border: 1px solid #E4EAF2; border-radius: 14px; overflow: hidden; box-shadow: 0 1px 3px rgba(1,20,66,0.04); }
        .am-head { padding: 10px 13px; border-bottom: 1px solid #EEF2F7; display: flex; align-items: center; gap: 8px; min-height:45px; }
        .am-head-icon { width: 27px; height: 27px; border-radius: 8px; background: rgba(13,64,252,0.07); display: flex; align-items: center; justify-content: center; font-size: 13px; }
        .am-head-title { font-size: 13px; font-weight: 700; color: #001442; }
        ...
        .am-grid2 { display: grid; grid-template-columns: 1fr 1fr; gap: 7px; margin-bottom: 9px; }
        ...
        .am-btn { width: 100%; background: #0D40FC; color: #fff; border: none; border-radius: 9px; padding: 9px; font-size: 12px; font-weight: 700; cursor: pointer; transition: all 0.18s; font-family: inherit; box-shadow: 0 2px 6px rgba(13,64,252,0.18); }
        ...
        .am-stats { display: flex; gap: 8px; margin-bottom: 9px; }
        .am-stat { flex:1;background: #F8FAFC; border: 1px solid #EEF2F7; border-radius: 9px; padding: 6px 9px; text-align: center;display:flex;align-items:center;justify-content:center;gap:7px; }
        .am-stat-val { font-size: 18px; font-weight: 800; color: #0D40FC; }
        .am-stat-val.booked { color: #F59E0B; }
        .am-stat-lbl { font-size: 11px; color: #8898AA; margin-top: 2px; }

        .am-filter { display: flex; align-items: center; gap: 8px; margin-bottom: 9px; }
        ...
        .am-date-group { margin-bottom: 9px; }
        ...
        .am-slot { display: flex; align-items: center; gap: 5px; padding: 4px 9px; border-radius: 16px; font-size: 11px; font-weight: 600; border: 1px solid; transition: all 0.15s; }
        ...
        .am-empty { text-align: center; padding: 16px; color: #94A3B8; font-size: 12px; }
        .am-empty-icon { font-size: 20px; margin-bottom: 4px; opacity: 0.3; }
        ...
        <div className="am-card">
          ...
          <div className="am-body">
            <form onSubmit={handleAdd}>
              ...
              {msg && (
                <div className={`am-msg ${isError ? "err" : "ok"}`}>{msg}</div>
              )}
              ...
              sortedDates.map((d) => (
                <div key={d} className="am-date-group">
                  <div className="am-date-label">
                    📆{" "}
                    {new Date(d + "T12:00:00").toLocaleDateString("ar-SA", {
                      weekday: "long",
                      year: "numeric",
                      month: "long",
                      day: "numeric",
                  ...
                  <div className="am-slots-grid">
                    {grouped[d]
                      ...
                      .map((slot) => (
                        <div
                          key={slot.id}
                          className={`am-slot ${slot.isBooked ? "booked" : "free"}`}
                        >
