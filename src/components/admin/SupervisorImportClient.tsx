...
export default function SupervisorImportClient({ embedded = false, onImported }: { embedded?: boolean; onImported?: () => void }) {
  ...
  return <main dir="rtl" style={{ maxWidth: 900, margin: "0 auto", padding: embedded ? 18 : 32 }}>
    ...
    <section style={{ background: "white", border: "1px solid #DBE5F1", borderRadius: 18, padding: 24 }}>
      ...
      {result?.preview && <label style={{display:"grid",gap:7,marginTop:16,fontSize:12,fontWeight:800,color:"#001442"}}>نبذة عن المشرف <span style={{color:"#DC2626"}}>إلزامية لإنشاء الصفحة التعريفية</span><textarea value={bio} onChange={(event)=>setBio(event.target.value)} rows={4} placeholder="اكتبي نبذة مهنية مختصرة تظهر في صفحة المشرف" style={{padding:12,border:"1px solid #CBD5E1",borderRadius:10,fontFamily:"inherit",resize:"vertical"}} required /></label>}
      <div style={{ display: "flex", gap: 10, marginTop: 16 }}><button disabled={!file || loading} onClick={() => void run(false)} style={{ padding: "11px 18px", borderRadius: 10, border: "1px solid #0D40FC", background: "white", color: "#0D40FC", fontWeight: 700,opacity:(!file||loading)?.55:1 }}>{loading?'جارٍ الفحص...':'معاينة وفحص'}</button><button disabled={!file || loading || !result?.preview || !bio.trim()} onClick={() => void run(true)} style={{ padding: "11px 18px", borderRadius: 10, border: 0, background: "#0D40FC", color: "white", fontWeight: 700,opacity:(!file||loading||!result?.preview||!bio.trim())?.55:1 }}>اعتماد الاستيراد وإنشاء الصفحة</button></div>
