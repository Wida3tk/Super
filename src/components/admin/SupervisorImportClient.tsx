...
export default function SupervisorImportClient({ embedded = false, onImported }: { embedded?: boolean; onImported?: () => void }) {
  ...
  async function run(commit: boolean) {
    if (!file || (commit && !bio.trim())) return;
    setLoading(true);
    const form = new FormData(); form.append("file", file); form.append("commit", String(commit)); form.append("bio", bio.trim());
  ...
  const errorLabels:Record<string,string>={INVALID_XLSX:'اختاري ملف Excel بصيغة xlsx.',FILE_TOO_LARGE:'حجم الملف يتجاوز 15MB.',UNSUPPORTED_SUPERVISOR_FILE:'هذا ليس قالب ملف المشرف المعتمد.',MISSING_SUPERVISOR_INFORMATION:'تعذر العثور على اسم المشرف أو بريده داخل الملف.',BIO_REQUIRED:'النبذة التعريفية إلزامية لإنشاء صفحة المشرف.',SUPERVISOR_ALREADY_EXISTS:'هذا المشرف موجود أصلًا. استخدمي زر «تحديث الساعات» بجانب حسابه؛ لن ننشئ حسابًا أو صفحة جديدة.',SUPERVISOR_IMPORT_FAILED:'تعذر قراءة الملف. تحققي من القالب وحاولي مجددًا.'};
  return <main dir="rtl" style={{ maxWidth: 900, margin: "0 auto", padding: embedded ? 18 : 32 }}>
    ...
    <section style={{ background: "white", border: "1px solid #DBE5F1", borderRadius: 18, padding: 24 }}>
      ...
      {result?.existingSupervisor && <div style={{marginTop:16,padding:13,borderRadius:10,background:'#FFF7ED',border:'1px solid #FED7AA',color:'#9A3412',fontSize:12}}><b>المشرف موجود بالفعل: {result.existingSupervisor.name}</b><div style={{marginTop:4}}>لن ننشئ حسابًا أو صفحة مكررة. عودي إلى «الحسابات والعمليات» واستخدمي زر «تحديث الساعات» بجانب حسابه.</div></div>}
      {result?.preview && !result?.existingSupervisor && <label style={{display:"grid",gap:7,marginTop:16,fontSize:12,fontWeight:800,color:"#001442"}}>نبذة عن المشرف <span style={{color:"#DC2626"}}>إلزامية لإنشاء الصفحة التعريفية</span><textarea value={bio} onChange={(event)=>setBio(event.target.value)} rows={4} placeholder="اكتبي نبذة مهنية مختصرة تظهر في صفحة المشرف" style={{padding:12,border:"1px solid #CBD5E1",borderRadius:10,fontFamily:"inherit",resize:"vertical"}} required /></label>}
      <div style={{ display: "flex", gap: 10, marginTop: 16 }}><button disabled={!file || loading} onClick={() => void run(false)} style={{ padding: "11px 18px", borderRadius: 10, border: "1px solid #0D40FC", background: "white", color: "#0D40FC", fontWeight: 700,opacity:(!file||loading)?.55:1 }}>{loading?'جارٍ الفحص...':'معاينة وفحص'}</button><button disabled={!file || loading || !result?.preview || !bio.trim() || Boolean(result?.existingSupervisor)} onClick={() => void run(true)} style={{ padding: "11px 18px", borderRadius: 10, border: 0, background: "#0D40FC", color: "white", fontWeight: 700,opacity:(!file||loading||!result?.preview||!bio.trim()||result?.existingSupervisor)?.55:1 }}>اعتماد الاستيراد وإنشاء الصفحة</button></div>
