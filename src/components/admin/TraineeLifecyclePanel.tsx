...
export default function TraineeLifecyclePanel({ trainees, supervisors, transitions }: { trainees: any[]; supervisors: any[]; transitions: any[] }) {
  ...
  return <div dir="rtl">
    <section style={{background:"linear-gradient(125deg,#001442,#0D40FC)",borderRadius:18,padding:20,color:"white",marginBottom:16,display:"flex",justifyContent:"space-between",gap:15,flexWrap:"wrap"}}><div><h2 style={{margin:"0 0 5px",fontSize:20}}>رحلة المتدربين التشغيلية</h2><p style={{margin:0,color:"#C7D7FF",fontSize:12}}>تظهر خدمات المنصة فقط للمتدرب الموجود في مرحلة البدء والاستمرار.</p></div><label style={{fontSize:12}}>حركة شهر <input type="month" value={month} onChange={e=>setMonth(e.target.value)} style={{display:"block",marginTop:5,padding:"8px 10px",borderRadius:9,border:0}}/></label></section>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(175px,1fr))",gap:10,marginBottom:18}}>{STAGES.map(stage=>{const total=trainees.filter(t=>stageOf(t)===stage.key).length;const entered=movements.filter(m=>m.toStage===stage.key).length;const left=movements.filter(m=>m.fromStage===stage.key).length;return <button key={stage.key} onClick={()=>setSelectedStage(selectedStage===stage.key?"all":stage.key)} style={{textAlign:"right",background:selectedStage===stage.key?"#F1F5FF":"white",border:`1px solid ${selectedStage===stage.key?stage.color:"#E2E8F0"}`,borderTop:`3px solid ${stage.color}`,borderRadius:14,padding:14,cursor:"pointer",fontFamily:"inherit"}}><div style={{display:"flex",justifyContent:"space-between"}}><span>{stage.icon}</span><b style={{fontSize:23,color:stage.color}}>{total}</b></div><div style={{fontWeight:800,color:"#001442",fontSize:12,margin:"7px 0"}}>{stage.short}</div><div style={{fontSize:10,color:"#64748B"}}>هذا الشهر: <span style={{color:"#059669"}}>+{entered} دخل</span> · <span style={{color:"#DC2626"}}>-{left} غادر</span></div></button>})}</div>
