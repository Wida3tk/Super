...
async function apiPatch(data: any) {
  ...
  if (!res.ok) {
    ...
    const messages: Record<string, string> = {
      EMAIL_ROLE_CONFLICT:
        "هذا البريد مستخدم لحساب مدير أو مشرف ولا يمكن استخدامه للمتدرب",
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          سيُضاف في مرحلة &quot;مقابلة أولية&quot; — الإسناد لمشرف يتم لاحقاً
          بعد التعاقد
        </p>
        <div
          style={{
          ...
          >
            {(["QASP-S", "QBA"] as License[]).map((l) => (
              <div
                ...
                style={{
                  padding: "12px",
                  border: `1px solid ${license === l ? COLORS.primary : COLORS.gray300}`,
                  borderRadius: 10,
                  cursor: "pointer",
                  textAlign: "center",
              ...
              >
                <div
                  style={{
                    fontSize: 15,
                    fontWeight: 600,
                    color: license === l ? "#0C447C" : COLORS.deep,
                  }}
                >
                  {l}
                </div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button
            ...
            style={{
              ...
              border: "none",
              borderRadius: 8,
              background: loading ? COLORS.gray300 : COLORS.deep,
              color: "#fff",
    ...
    return (
      ...
      >
        ...
        >
          ...
          <h3 style={{ margin: "0 0 6px", color: COLORS.deep }}>
            تم الإسناد وإنشاء الحساب
          </h3>
          <p style={{ fontSize: 13, color: COLORS.gray500, lineHeight: 1.7 }}>
            تم إنشاء رابط تعيين كلمة المرور للمتدرب. انسخي الرابط وأرسليه له إذا
            لم تكن خدمة البريد مفعلة.
          </p>
          <div
            dir="ltr"
            style={{
                ...
                <div style={{ flex: 1 }}>
                  <div
                    style={{
                      fontSize: 13,
                      fontWeight: 500,
                      color: selectedSup === s.id ? "#0C447C" : COLORS.deep,
                    }}
                  >
                    {s.name}
                  </div>
                  <div style={{ fontSize: 11, color: COLORS.gray500 }}>
                    {s.availableSeats ?? 0} مقعد متاح
                  </div>
                </div>
              </div>
            ))}
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button
            ...
            style={{
              ...
              border: "none",
              borderRadius: 8,
              background: loading ? COLORS.gray300 : COLORS.success,
              color: "#fff",
              cursor: loading ? "not-allowed" : "pointer",
              fontSize: 13,
              fontWeight: 600,
...
async function exportToExcel(
  ...
  if (type === "supervisors") {
    const summaryData: CsvRow[] = supervisors.map((sup) => {
      ...
      const totalInd = supSnaps.reduce(
        (a, s) => a + (s.individualHours || 0),
        0,
      );
...
export default function AdminSupervisionPanel({
  ...
  const [activeTab, setActiveTab] = useState<
    "supervisors" | "trainees" | "onboarding" | "months"
  >("supervisors");
  const [selectedMonth] = useState(new Date().toISOString().slice(0, 7));
  const [showAddModal, setShowAddModal] = useState(false);
  const [assigningTrainee, setAssigningTrainee] = useState<Trainee | null>(
    null,
  );
  const [agreementTrainee, setAgreementTrainee] = useState<Trainee | null>(
    null,
  );
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button
            onClick={() => handleExport("supervisors")}
        ...
        >
          ...
          >
            <span style={{ fontSize: 13, fontWeight: 600, color: COLORS.deep }}>
              تقرير الإنتاجية — {formatMonth(selectedMonth)}
            </span>
            ...
            >
              <thead>
                <tr style={{ background: COLORS.gray100 }}>
                  {["المشرف", "المتدربون", "فردية", "جماعية", "الإجمالي"].map(
                    (h) => (
                      <th
                        key={h}
                        style={{
              ...
              <tbody>
                {supervisors.map((sup) => {
                  ...
                  const totalGrp = supSnaps.reduce(
                    (a, s) => a + (s.groupHours || 0),
                    0,
                  );
                  const traineeCount = trainees.filter(
                    (t) =>
                      t.currentSupervisorId === sup.id && t.status === "active",
                  ).length;
                  return (
                    <tr
                      key={sup.id}
                        ...
                        <td style={{ padding: "12px 16px" }}>
                          <select
                            ...
                            onChange={(e) =>
                              handleChangeStatus(
                                t.id,
                                e.target.value as TraineeStatus,
                              )
                            }
                            style={{
                              padding: "4px 8px",
                          ...
                          >
                            {(Object.keys(STATUS_LABELS) as TraineeStatus[])
                          ...
                          {ONBOARDING_STAGES.map((stage) => {
                            ...
                            return (
                              <button
                                key={stage.key}
                                onClick={() =>
                                  !isCurrent && handleSetStage(t.id, stage.key)
                                }
                                disabled={isLoading}
                                style={{
                                  padding: "5px 10px",
      ...
      {activeTab === "months" && (
        <div>
          ...
          >
            ...
            <div>
              ...
              <p style={{ fontSize: 12, marginTop: 2, opacity: 0.85 }}>
                مع بداية كل شهر جديد، يُقفل الشهر السابق تلقائياً. يمكن للأدمن
          ...
          >
            ...
            >
              <span
                style={{ fontSize: 13, fontWeight: 600, color: COLORS.deep }}
              >
                إدارة الأشهر — {formatMonth(selectedMonth)}
              </span>
            </div>
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {snapshots.map((snap) => {
                    const sup = supervisors.find(
                      (s) => s.id === snap.supervisorId,
                    );
                    const trainee = trainees.find(
                      (t) => t.id === snap.traineeId,
                    );
                    ...
                    return (
                      ...
                      >
                        ...
                        <td style={{ padding: "12px 16px" }}>
                          {isLocked ? (
                            <Badge
                              label={`🔒 مقفول${snap.lockedBy === "auto" ? " تلقائياً" : ""}`}
                              bg={COLORS.gray100}
                              color={COLORS.gray500}
                            />
                          ) : (
      ...
      {showAddModal && (
        <AddTraineeModal onClose={() => setShowAddModal(false)} />
      )}
      {assigningTrainee && (
        <AssignModal
      ...
      {agreementTrainee && (
        <AdminAgreementModal
          trainee={agreementTrainee}
          onClose={() => setAgreementTrainee(null)}
        />
      )}
    </div>
  );
