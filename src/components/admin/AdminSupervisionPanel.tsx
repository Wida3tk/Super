...
function AddTraineeModal({
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await onSubmit({ name, email, phone, license });
      onClose();
    } catch (e: any) {
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {(["QASP-S", "QBA"] as License[]).map((l) => (
              <div key={l} onClick={() => setLicense(l)}
                style={{
                  padding: "12px", border: `1px solid ${license === l ? COLORS.primary : COLORS.gray300}`,
                  borderRadius: 10, cursor: "pointer", textAlign: "center",
                  background: license === l ? "#E6F1FB" : "#fff",
                }}>
...
function AssignModal({
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await onSubmit({ traineeId: trainee.id, supervisorId: selectedSup, startDate });
      onClose();
    } catch (e: any) {
      setError(e.message || "حدث خطأ");
    } finally {
            ...
            {supervisors.map((s) => (
              <div key={s.id} onClick={() => setSelectedSup(s.id)}
                style={{
                  ...
                  border: `1px solid ${selectedSup === s.id ? COLORS.primary : COLORS.gray300}`,
                  borderRadius: 10, cursor: "pointer",
                  background: selectedSup === s.id ? "#E6F1FB" : "#fff",
                }}>
  ...
  if (type === "supervisors") {
    ...
    const summaryData = supervisors.map((sup) => {
      ...
      return {
        "المشرف": sup.name,
        "الإيميل": sup.email,
        "عدد المتدربين": trainees.filter((t) => t.currentSupervisorId === sup.id).length,
        "فردية (ساعة)": totalInd,
        "جماعية (ساعة)": totalGrp,
        "الإجمالي": totalInd + totalGrp,
      };
    });
    ...
    supervisors.forEach((sup) => {
      ...
      if (data.length > 0) {
        ...
        const sheetName = sup.name?.slice(0, 31) || sup.id;
        XLSX.utils.book_append_sheet(wb, sheet, sheetName);
      }
    });

    XLSX.writeFile(wb, `تقرير-المشرفين-${selectedMonth || "كامل"}.xlsx`);
    ...
    const summaryData = trainees.map((t) => {
      ...
      return {
        ...
        "الرخصة": t.license,
        "الساعات المطلوبة": t.requiredHours,
        "فردية": t.totalIndividualHours,
        "جماعية": t.totalGroupHours,
        "الإجمالي": t.totalHours,
        "المتبقي": t.requiredHours - t.totalHours,
    ...
    // ورقة لكل متدرب
    trainees.forEach((t) => {
      const tSnapshots = snapshots.filter((s) => s.traineeId === t.id);
      if (tSnapshots.length > 0) {
        const data = tSnapshots.map((s) => ({
          "الشهر": formatMonth(s.month),
          "ساعات العمل": s.workHours,
          "المطلوب (5%)": s.requiredHours,
          "فردية": s.individualHours,
          "جماعية": s.groupHours,
          "الإجمالي": s.totalHours,
          "نسبة الجماعي %": s.groupPercentage,
          "غيابات": s.absenceCount,
          "إنذارات": s.warningCount,
          "الشهر مقفول": s.lockedAt ? "نعم" : "لا",
        }));
        const sheet = XLSX.utils.json_to_sheet(data);
        const sheetName = t.name?.slice(0, 31) || t.id;
        XLSX.utils.book_append_sheet(wb, sheet, sheetName);
      }
    });

    XLSX.writeFile(wb, `تقرير-المتدربين-${selectedMonth || "كامل"}.xlsx`);
  }
}
...
// Main Component
// ===========================
export default function AdminSupervisionPanel({ supervisors }: { supervisors: any[] }) {
  const {
    trainees, loading, error, stats,
    createTrainee, assignTraineeToSupervisor,
    advanceOnboarding, changeTraineeStatus,
    lockMonthForTrainee, unlockMonthForTrainee,
    refresh,
  } = useAdminSupervision();
  ...
  // جلب الـ snapshots لكل المشرفين
  useEffect(() => {
    const fetchSnapshots = async () => {
      const all: MonthlySnapshot[] = [];
      for (const sup of supervisors) {
        const snaps = await getSnapshotsBySupervisorMonth(sup.id, selectedMonth);
        all.push(...snaps);
      }
      setSnapshots(all);
    };
    fetchSnapshots();
  }, [selectedMonth, supervisors]);

  const handleExport = async (type: "supervisors" | "trainees") => {
    setExportLoading(true);
    try {
      await exportToExcel(type, trainees, supervisors, snapshots, selectedMonth);
    } finally {
  ...
  if (loading) return <div style={{ padding: "2rem", textAlign: "center", color: COLORS.gray500 }}>جاري التحميل...</div>;
  if (error) return <div style={{ padding: "2rem", textAlign: "center", color: COLORS.danger }}>{error}</div>;
  ...
  return (
    <div style={{ direction: "rtl", fontFamily: "'IBM Plex Sans Arabic', sans-serif" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1.5rem", flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 700, color: COLORS.deep }}>الإشراف الأكاديمي</h1>
          <p style={{ fontSize: 12, color: COLORS.gray500, marginTop: 3 }}>لوحة الإدارة — سلوكيرا</p>
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button onClick={() => handleExport("supervisors")} disabled={exportLoading}
            style={{ padding: "8px 14px", border: `1px solid ${COLORS.gray300}`, borderRadius: 10, background: "#fff", color: COLORS.gray500, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
            📊 تصدير المشرفين
          </button>
          ...
          <button onClick={() => setShowAddModal(true)}
            style={{ padding: "8px 16px", border: "none", borderRadius: 10, background: COLORS.deep, color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
            + إضافة متدرب
          </button>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: "1.5rem" }}>
        {[
          ...
          <div key={s.label} style={{ background: "#fff", borderRadius: 12, padding: "14px 16px", border: `1px solid ${COLORS.gray200}` }}>
        ...
        {tabs.map((t) => (
          <button key={t.key} onClick={() => setActiveTab(t.key as any)}
            style={{
              ...
              background: "none", color: activeTab === t.key ? COLORS.primary : COLORS.gray500,
              fontWeight: activeTab === t.key ? 600 : 400, fontFamily: "inherit",
            }}>
            {t.label}
          </button>
        ))}
      ...
      {/* Month Filter */}
      {(activeTab === "supervisors" || activeTab === "months") && (
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 700 }}>
              <thead>
                <tr style={{ background: COLORS.gray100 }}>
                  {["المشرف", "المتدربون", "فردية", "جماعية", "الإجمالي", "الشهر"].map((h) => (
                    ...
                    <tr key={sup.id} style={{ borderBottom: `1px solid ${COLORS.gray200}` }}>
                      <td style={{ padding: "12px 16px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#E6F1FB", color: "#0C447C", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 600 }}>
                            {sup.name?.slice(0, 2)}
                          </div>
                          <div>
                      ...
                      <td style={{ padding: "12px 16px", fontSize: 15, fontWeight: 700, color: COLORS.deep }}>{totalInd + totalGrp}</td>
                      <td style={{ padding: "12px 16px" }}>
                        <Badge label={formatMonth(selectedMonth)} bg={COLORS.gray100} color={COLORS.gray500} />
                      </td>
                    </tr>
                  );
                })}
      ...
      {activeTab === "trainees" && (
        <div style={{ background: "#fff", borderRadius: 14, border: `1px solid ${COLORS.gray200}`, overflow: "hidden" }}>
          <div style={{ padding: "12px 16px", borderBottom: `1px solid ${COLORS.gray200}`, background: COLORS.gray100, display: "flex", justifyContent: "space-between" }}>
            ...
            <span style={{ fontSize: 12, color: COLORS.gray500 }}>{trainees.filter(t => t.status !== "onboarding").length} متدرب</span>
          </div>
          <div style={{ overflowX: "auto" }}>
                    ...
                    <tr key={t.id} style={{ borderBottom: `1px solid ${COLORS.gray200}` }}>
                      <td style={{ padding: "12px 16px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#E6F1FB", color: "#0C447C", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 600 }}>
                            {t.name?.slice(0, 2)}
                          </div>
                          <div>
                      ...
                      <td style={{ padding: "12px 16px" }}>
                        <select value={t.status} onChange={(e) => changeTraineeStatus(t.id, e.target.value as TraineeStatus)}
                ...
                {trainees.filter(t => t.status === "onboarding").map((t) => {
                  ...
                  return (
                    <tr key={t.id} style={{ borderBottom: `1px solid ${COLORS.gray200}` }}>
                      <td style={{ padding: "12px 16px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#F1EFE8", color: "#5F5E5A", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 600 }}>
                            {t.name?.slice(0, 2)}
                          </div>
                          <div>
                            <div style={{ fontSize: 13, fontWeight: 500 }}>{t.name}</div>
                      ...
                      <td style={{ padding: "12px 16px" }}>
                        <Badge
                          label={t.onboardingStage ? ONBOARDING_LABELS[t.onboardingStage] : "—"}
                          bg={isReady ? "#EEEDFE" : "#F1EFE8"}
                          color={isReady ? "#3C3489" : "#5F5E5A"}
                        />
                      </td>
                      <td style={{ padding: "12px 16px" }}>
                        <div style={{ display: "flex", gap: 6 }}>
                          {nextStage && (
                            <button onClick={() => advanceOnboarding(t.id, nextStage)}
      ...
      {activeTab === "months" && (
        <div>
          <div style={{ background: "#E6F1FB", border: `1px solid #85B7EB`, borderRadius: 10, padding: "10px 14px", marginBottom: "1rem", display: "flex", gap: 10, alignItems: "flex-start", fontSize: 13, color: "#185FA5" }}>
            <span>ℹ️</span>
            <div>
                  ...
                  {snapshots.map((snap) => {
                    ...
                    return (
                      <tr key={snap.id} style={{ borderBottom: `1px solid ${COLORS.gray200}`, opacity: isLocked ? 0.75 : 1 }}>
                        ...
                        <td style={{ padding: "12px 16px" }}>
                          {isLocked ? (
                            <button onClick={() => unlockMonthForTrainee(snap.supervisorId, snap.traineeId, snap.month)}
                              style={{ padding: "5px 10px", border: `1px solid ${COLORS.success}`, borderRadius: 8, background: "#E1F5EE", color: COLORS.success, fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>
                              فتح
                            </button>
                          ) : (
                            <button onClick={() => lockMonthForTrainee(snap.supervisorId, snap.traineeId, snap.month)}
                              style={{ padding: "5px 10px", border: `1px solid ${COLORS.gray300}`, borderRadius: 8, background: COLORS.gray100, color: COLORS.gray500, fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>
                              قفل
                            </button>
                          )}
                        </td>
      ...
      {showAddModal && (
        <AddTraineeModal
          ...
          onSubmit={createTrainee}
        />
      )}
      {assigningTrainee && (
        <AssignModal
          trainee={assigningTrainee}
          supervisors={supervisors}
          onClose={() => setAssigningTrainee(null)}
          onSubmit={assignTraineeToSupervisor}
        />
      )}
    </div>
  );
