...
export default function SupervisorHoursUpdate({ supervisor, onClose }: { supervisor: Supervisor; onClose: () => void }) {
  ...
  const errors: Record<string, string> = {
    ...
    NO_MATCHED_TRAINEES: "لا يوجد في الملف متدرب مرتبط حاليًا بهذا المشرف داخل النظام.",
  ...
  return <div className="hours-overlay" dir="rtl" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    ...
    <section className="hours-dialog">
      ...
      <small className="hours-kicker">تحديث آمن دون إنشاء حساب</small>
      <h2>تحديث ساعات {supervisor.name}</h2>
      <p className="hours-email">{supervisor.email}</p>
      <div className="hours-safety"><b>لن ننشئ حسابًا أو صفحة جديدة.</b><span>سنطابق بريد المشرف، ثم نحدّث ساعات المتدربين المرتبطين به فقط. السجلات الموجودة لن تتكرر.</span></div>
      ...
      <div className="hours-actions">
        ...
        <button className="commit" disabled={!file || loading || !result?.preview || result?.committed} onClick={() => void run(true)}>اعتماد تحديث الساعات</button>
      </div>
      {result?.preview && <div className="hours-report">
        ...
        <div className="report-stats"><div><b>{result.preview.matched.length}</b><span>متدرب مطابق</span></div><div className={result.preview.unmatched.length ? "warn" : ""}><b>{result.preview.unmatched.length}</b><span>غير مرتبط</span></div><div><b>{result.preview.matched.reduce((sum: number, row: any) => sum + Number(row.records || 0), 0)}</b><span>سجل ساعة بالملف</span></div></div>
        ...
        {!!result.preview.unmatched.length && <details><summary>متدربون لن يتم استيرادهم ({result.preview.unmatched.length})</summary><p>هؤلاء غير مرتبطين حاليًا بالمشرف في النظام؛ لن ننشئ لهم حسابات تلقائيًا.</p>{result.preview.unmatched.map((row: any) => <span className="unmatched" key={row.email}>{row.name} — {row.email}</span>)}</details>}
        {result.committed && <p className="success">تمت المحافظة على الحساب والصفحة التعريفية الحالية، وتحديث السجلات المطابقة فقط.</p>}
      </div>}
