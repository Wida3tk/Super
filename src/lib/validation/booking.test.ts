...
describe("booking validation", () => {
  ...
  it("rejects missing slot identity", () => {
    expect(
      validateBookingPayload({ ...validPayload, availabilitySlotId: "" }),
    ).toBe("INVALID_SLOT");
  });

  it("rejects malformed contact data", () => {
    expect(
      validateBookingPayload({ ...validPayload, studentEmail: "invalid" }),
    ).toBe("INVALID_EMAIL");
    expect(
      validateBookingPayload({ ...validPayload, studentPhone: "123" }),
    ).toBe("INVALID_PHONE");
  });
