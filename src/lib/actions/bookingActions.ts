...
import { isManagementToken, validateBookingPayload } from '@/lib/validation/booking';
...
export async function createBooking(
  ...
  try {
    const supervisor = await adminDb.runTransaction(async transaction => {
      ...
      if (!existingBookings.empty) throw new Error('ALREADY_HAS_BOOKING');
      ...
      if (supervisorData.isActive === false) throw new Error('SUPERVISOR_NOT_FOUND');
      if ((supervisorData.availableSeats ?? 0) <= 0) throw new Error('NO_SEATS_AVAILABLE');
      if (
        slot.isBooked ||
    ...
    return { success: false, error: BOOKING_ERRORS.has(message) ? message : 'UNKNOWN_ERROR' };
  }
}
...
export async function cancelBookingByToken(
  ...
  try {
    const booking = await adminDb.runTransaction(async transaction => {
      ...
      transaction.update(adminDb.collection('supervisors').doc(data.supervisorId), {
        availableSeats: FieldValue.increment(1),
      });

      if (data.availabilitySlotId) {
        transaction.update(adminDb.collection('availability').doc(data.availabilitySlotId), {
          isBooked: false,
          bookingId: null,
        });
      }
